...
final class AudioStreamer {
    ...
    /// Called on an audio thread with the raw mic buffer (hardware format), for a
    /// second consumer (e.g. Apple's recognizer in compare mode).
    ...
    func start() throws {
        ...
        input.installTap(onBus: 0, bufferSize: 4096, format: hwFormat) { [weak self] buf, _ in
            self?.onBuffer?(buf)   // raw buffer to the second consumer (Apple)
            self?.handle(buf)      // resample + RMS for Parakeet
        }
        engine.prepare()
        try engine.start()
    }
