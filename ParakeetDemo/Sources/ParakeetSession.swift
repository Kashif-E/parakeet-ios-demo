...
final class ParakeetSession {
    ...
    /// Compile the Metal compute kernels up front by running ~1 s of silence
    /// through a throwaway stream. Without this the per-kernel JIT happens during
    /// the user's first real utterance (visible lag); doing it at load hides the
    /// cost behind the "Loading model…" state. Heavy — call off the main thread.
    func warmup() {
        queue.sync {
            guard let s = parakeet_capi_stream_begin_lang(ctx, "en") else { return }
            let silence = [Float](repeating: 0, count: 16000)
            var mask: Int32 = 0
            silence.withUnsafeBufferPointer {
                if let t = parakeet_capi_stream_feed(s, $0.baseAddress, Int32(silence.count), &mask) {
                    parakeet_capi_free_string(t)
                }
            }
            if let t = parakeet_capi_stream_finalize(s) { parakeet_capi_free_string(t) }
